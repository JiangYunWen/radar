<template>
  <div>
    <div class="cont">
      ...
      <div class="contsolt" v-for="index in 14" :key="index" solt="getslotobject(index)">
        ...
        <template v-else-if="getslotobject(index).subtype === 5">
          ...
          >
            <el-checkbox
              :label="getslotobject(index).deviceId"
              :key="getslotobject(index).deviceId"
            >
              <span hidden> {{ getslotobject(index).deviceId }}</span>
            </el-checkbox>
          </el-checkbox-group>
          <span class="name1" v-if="getslotobject(index).name.length < 5">{{
            getslotobject(index).name
          }}</span>
          <span class="name" v-else>{{ getslotobject(index).name }}</span>
        </template>
            ...
            <template v-if="getslotobject(index).alive === true">
              ...
              <SoltStatus
                v-bind:showdata="{
                  titlename: 'CPU',
                  titleclass: 'cpu',
                  msgclass: 'bat fa fa-fw',
                  showico: 'fa-battery-',
                  value: getslotobject(index).status.brt_bmc_sys.cpurate,
                }"
                v-if="getslotobject(index).status.brt_bmc_sys.cpurate !== undefined"
              >
              </SoltStatus>
              ...
              <SoltStatus
                v-bind:showdata="{
                  titlename: '内存',
                  titleclass: 'memory',
                  msgclass: 'memorybat fa fa-fw',
                  showico: 'fa-battery-',
                  value: getslotobject(index).status.brt_bmc_sys.memRate,
                }"
                v-if="getslotobject(index).status.brt_bmc_sys.memRate !== undefined"
              >
              </SoltStatus>
              ...
              <SoltStatus
                v-bind:showdata="{
                  titlename: 'SSD',
                  titleclass: 'ssd',
                  msgclass: 'ssdbat fa fa-fw',
                  showico: 'fa-battery-',
                  value: getslotobject(index).status.brt_bmc_sys.SSDRate,
                }"
                v-if="!isNaN(getslotobject(index).status.brt_bmc_sys.SSDRate)"
              >
              </SoltStatus>
    ...
    <div class="filter-container">
      ...
      <div
        class="right-items"
        style="float: right; margin: 5px -10px; position: relative"
        hidden
      >
...
export default {
    ...
    msgTolist() {
      ...
      for (var i = 0; i < this.msg.length; i++) {
        if (this.msg[i].status.brt_bmc_sys != null) {
          this.msg[i].status.brt_bmc_sys["ssdRate"] = Math.round(
            (this.msg[i].status.brt_bmc_sys.usedSSDSize /
              this.msg[i].status.brt_bmc_sys.ssdsize) *
              100
          );
  ...
  mounted: function () {
    ...
    $subBox.click(function () {
      $("#checkAll").attr(
        "checked",
        $subBox.length == $("input[name='check']:checked").length ? true : false
      );
    });
